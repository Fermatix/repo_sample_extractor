...
async def _exec_bash(ctx: _ToolCtx, command: str) -> str:
    ...
    if ctx.primary_language:
        ...
        pct = round(ploc / saved_loc * 100) if saved_loc else 0
        progress += (
...
def _exec_save_sample(
    ...
    if ctx.primary_language and running_total:
        result["primary_share"] = round(_primary_loc(ctx) / running_total, 2)
    return result
...
async def run_agent(
    ...
    for iteration in range(settings.agent_max_iterations):
        ...
        if saves_so_far == 0 and iteration >= 3:
            ...
            pct = round(primary_loc / saved_loc * 100) if saved_loc else 0
            need = max(0, int(settings.primary_share_min * saved_loc) - primary_loc)
            exts = ", ".join(extensions_for(ctx.primary_language)) or "n/a"
            headroom = (
                f" The {settings.max_total_loc} LOC hard cap leaves room even though "
                f"you are at target."
            ...
            nudge = (
                ...
                f"{int(settings.primary_share_min * 100)}%. You need ~{need} more LOC of "
