...
def _build_system_prompt(settings: Settings, has_primary: bool = True) -> str:
    language_section = f"""
...
async def _exec_bash(ctx: _ToolCtx, command: str) -> str:
    ...
    if ctx.primary_language:
        ...
        progress += (
            ...
            f" — min {int(ctx.settings.primary_share_min * 100)}%"
        )
    output += f"\n{progress}]"
    return output
...
async def run_agent(
    ...
    primary = settings.primary_language_override.strip() or (stats.primary or "")
    if primary and not primary_forced and not is_trackable(primary):
        # scc can report languages our path-based tagging cannot recognize
        # (e.g. Solidity, JSP). Enforcing one of those would count 0 LOC
        # forever and reject every run — fall back to the best language we
        # CAN track, or disable enforcement.
        fallback = next(
            (lang for lang, _ in sorted(stats.counts.items(), key=lambda kv: -kv[1])
             if is_trackable(lang)),
            "",
        )
        logger.warning(
            f"[{repo_name}] detected primary '{primary}' is not trackable by "
            f"file path — {'falling back to ' + repr(fallback) if fallback else 'disabling the language requirement'}"
        )
        primary = fallback
    ctx.primary_language = primary or None
    if primary_forced and stats.counts.get(primary, 0) == 0:
        logger.warning(
            f"[{repo_name}] forced primary language '{primary}' not detected in the "
            f"repo scan ({stats.source}) — enforcing anyway"
        )
    logger.info(
    ...
    if primary:
        ...
        first_user_msg = (
            ...
            f"HARD REQUIREMENT: at least {share_min_pct}% of the LOC you save must be "
            f"{primary} files. Track this in the LOC progress shown after each tool "
            f"call.\n\nBegin."
        )
    else:
        first_user_msg = (
            f"Repository has been cloned to: {repo_path}\n\n"
            "Language distribution could not be determined — the Language coverage "
            "minimum does not apply. Identify the dominant real source language "
            "yourself and focus the sample on it.\n\nBegin."
        )

    messages: list[dict] = [
        {"role": "system", "content": _build_system_prompt(settings, has_primary=bool(primary))},
    ...
    for iteration in range(settings.agent_max_iterations):
        ...
        if saves_so_far == 0 and iteration >= 3:
            ...
            # primary-language minimum guarantees post-run rejection.
            ...
            if need > cap_headroom:
                # Even filling all remaining cap headroom with primary files
                # cannot reach the minimum — further iterations are wasted;
                # end the run, the retry starts over with a clean budget.
                note = (
            ...
            headroom = (
                f" The {settings.max_total_loc} LOC hard cap still has {cap_headroom} LOC "
                f"of room even though you are at target."
                if saved_loc >= settings.target_loc else ""
            )
            nudge = (
                f"⚠️ LANGUAGE REQUIREMENT FAILING: only {primary_loc} LOC ({pct}%) of your "
                f"sample is {ctx.primary_language} — the minimum is "
                f"{int(share_min * 100)}%. You need ~{need} more LOC of "
                f"hand-written {ctx.primary_language} files ({exts}). Save those NOW.{headroom} "
            ...
            # Suppressed while the primary-language minimum is unmet — telling
            # the agent to finish in that state would guarantee rejection.
            nudge = (
            ...
            lang_caveat = (
                ...
                f"{int(settings.primary_share_min * 100)}% minimum — prioritize "
            ...
            lang_caveat = (
                f" Your {ctx.primary_language} share is below the "
                f"{int(settings.primary_share_min * 100)}% minimum — those last saves "
                f"must be {ctx.primary_language} files."
                if ctx.primary_language
                and primary_loc < settings.primary_share_min * saved_loc else ""
            )
            nudge = (
